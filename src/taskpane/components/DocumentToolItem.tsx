...
export function DocumentToolItem({
  ...
  const actionBusy = loading || companionLoading;
  const runDisabled = disabled || actionBusy || blockedBySelection || missingRequiredText;
  const companionDisabled = disabled || actionBusy || !onRunCompanion;
  ...
  return (
    <article className={`document-tool-item${loading ? " loading" : ""}`}>
      ...
      {tool.fields?.length ? (
        <div className="document-tool-item-fields">
          {tool.fields.map((field) =>
            renderField(field, fieldValues[field.key] ?? field.defaultValue, disabled || actionBusy, onFieldChange)
          )}
        </div>
      ) : null}
      ...
      <div className="document-tool-item-actions">
        ...
        <div className="document-tool-action-buttons">
          <button type="button" className="document-tool-run-btn" disabled={runDisabled} onClick={onRun}>
            {loading ? "处理中…" : tool.actionLabel || "执行"}
          </button>
          {companionTool && onRunCompanion ? (
            <button
              type="button"
              className="document-tool-secondary-btn"
              disabled={companionDisabled}
              title={companionTool.description}
              onClick={onRunCompanion}
            >
              {companionLoading ? "处理中…" : companionTool.actionLabel || "清除"}
            </button>
          ) : null}
        </div>
      </div>
